...
function viewEventRequests() {
	...
	try {	
		...
		$sql = "SELECT er.EventRequestId, er.EventId, e.EventName, e.UserId AS OwnerId, e.StartTime, e.EndTime, e.EventDescription FROM EventRequest er INNER JOIN Events e ON er.EventId=e.EventId WHERE er.UserId=:userId";
		$stmt = $db->prepare($sql);
		$stmt->bindParam('userId', $userId);
		$stmt->execute();
